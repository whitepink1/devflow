    ...
    <section className="background-light900_dark200 light-border sticky right-0 top-0 flex h-screen flex-col overflow-y-auto border-l p-6 pt-36 shadow-light-300 dark:shadow-none max-xl:hidden w-[350px]">
        <div>
            ...
            <div className="mt-7 flex w-full flex-col gap-[30px]">
                {hotQuestions?.map((question) => (
                    <Link key={question.title} href={`/question/${question._id}`} className="flex cursor-pointer items-center justify-between gap-7">
        ...
        <div className="mt-16">
            ...
            <div className="mt-7 flex flex-col gap-4">
                {popularTags?.map((tag) => (
                    <RenderTag key={tag._id} _id={tag._id} name={tag.name} totalQuestions={tag.numberOfQuestions} showCount={true}/>
                ))}
            </div>
        </div>
    </section>)
